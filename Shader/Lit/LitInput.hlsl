...
*/
    ...
    #define SAMPLE_METALLICSPECULAR(uv) SAMPLE_TEXTURE2D(_SpecularMap, sampler_SpecularMap, uv)
#else
    #define SAMPLE_METALLICSPECULAR(uv) SAMPLE_TEXTURE2D(_MetallicMap, sampler_MetallicMap, uv)
#endif
...
half4 SampleMetallicSpecGloss(float2 uv, half albedoAlpha)
{
    ...
    #ifdef _SPECULAR
        float3 _SpecColor = SAMPLE_TEXTURE2D(_SpecularMap, sampler_SpecularMap, uv);
...
inline void InitializeStandardLitSurfaceData(float2 uv, out SurfaceData outSurfaceData)
{
    ...
    outSurfaceData.normalTS = SampleNormal(uv, TEXTURE2D_ARGS(_NormalTex, sampler_NormalTex), _NormalPower);
    outSurfaceData.occlusion = SampleOcclusion(uv);
    outSurfaceData.emission = SampleEmission(uv, _Emission.rgb, TEXTURE2D_ARGS(_EmissionMap, sampler_EmissionMap));
